"""
feed.py — orchestrates chunking, embedding, and indexing of markdown files into Vespa.
...
from backend.vespa_utils import wait_for_vespa, deploy_app, feed_all, file_already_indexed
from backend.extract_all_md_files import list_markdown_paths
import logging
...
def main(include_folders=None, exclude_folders=None) -> None:
    ...
    if not md_paths:
        logging.info("Usage: uv run python -m backend.feed")
        logging.info("   or: uv run python -m backend.feed --include_folders='agents'")
        sys.exit(1)
